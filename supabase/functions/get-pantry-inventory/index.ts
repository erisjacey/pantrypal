...
import type { Unit } from '../_shared/domain/Unit.ts'
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const [key, batches] of grouped) {
      const domainItems = batches.map((b) =>
        Item.create({
          id: b.id,
          name: b.name,
          currentQuantity: Quantity.create(b.current_quantity, b.unit as Unit),
          initialQuantity: Quantity.create(b.initial_quantity, b.unit as Unit),
